...
final class LottieViewController: UIViewController {
    ...
    @IBOutlet private weak var animationContainerView1: UIView! {
        didSet {
            guard let animation = Animation.named("9965-loading-spinner", subdirectory: nil) else {
    ...
    @IBOutlet private weak var animationContainerView2: UIView! {
        didSet {
            guard let animation = Animation.named("32910-success", subdirectory: nil) else {
                print("\(#line) file not found")
                return
            }
    ...
    @IBOutlet private weak var animationContainerView3: UIView! {
        didSet {
            guard let animation = Animation.named("32921-volumn-from-mute-to-maximum", subdirectory: nil) else {
                print("\(#line) file not found")
                return
            }
